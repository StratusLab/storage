...
register_disk() {
    # Do by python script stratus-pdisk-client.py
    if [ "x$SCRIPT" != "xpython" ]
    then
	echo "!!!!! THIS IS OBSOLETE USAGE !!! Use python script instead (add SCRIPT=python from /etc/stratuslab/pdisk-host.cfg)"
...
attach_iscsi() {
	...
	if [ "x$TARGET" != "x" ]
	then
    		/usr/sbin/stratus-pdisk-client.py --username $PDISK_USER --password $PDISK_PSWD --pdisk-id $UUID_URL --vm-dir $vms_dir --vm-id $VM_ID --vm-disk-name $device_name --target $TARGET --attach --link --mount --op up
	else
    		/usr/sbin/stratus-pdisk-client.py --username $PDISK_USER --password $PDISK_PSWD --pdisk-id $UUID_URL --vm-dir $vms_dir --vm-id $VM_ID --vm-disk-name $device_name --register --attach --link --op up
	fi
    else
