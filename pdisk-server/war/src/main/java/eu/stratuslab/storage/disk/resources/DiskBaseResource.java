...
public class DiskBaseResource extends BaseResource {
    ...
    protected Disk processWebForm(Disk disk, Form form) {
        ...
        if (sizeInForm != null) {
            ...
            try {
                ...
                throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, "Error parsing size: " + e.getMessage());
            }
        ...
        if (visibilityInForm != null) {
            ...
            try {
                visibility = DiskVisibility.valueOf(visibilityInForm.toUpperCase());
    ...
    protected void validateNewDisk(Disk disk) {
        ...
        if (disk.getIdentifier() == null) {
            throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN, "Invalid UUID provided");
        }

        if (Disk.identifierExists(disk.getIdentifier())) {
            throw new ResourceException(Status.CLIENT_ERROR_FORBIDDEN, "Disk already registered");
        }

        long gigabytes = disk.getSize();

        if (gigabytes < ServiceConfiguration.DISK_SIZE_MIN || gigabytes > ServiceConfiguration.DISK_SIZE_MAX) {
            String error = "Size must be an integer between " + ServiceConfiguration.DISK_SIZE_MIN + " and " +
    ...
    protected Disk loadExistingDisk() {
        ...
        if (disk == null) {
            throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND, "disk (" + getDiskId() + ") does not exist");
        }
        return disk;
    }
    ...
    private void checkViewRightsOrError(Disk disk) {
        if (!hasSufficientRightsToView(disk)) {
            throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND, "disk (" + disk.getUuid() + ") does not exist");
        }
    }
    ...
    protected void addCreateFormDefaults(Map<String, Object> info) {
        ...
        defaults.put(Disk.DISK_VISIBILITY_KEY, DiskVisibility.PRIVATE.toString());
